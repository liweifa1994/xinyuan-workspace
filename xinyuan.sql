...
Source Host           : 10.100.70.144:3306
...
CREATE TABLE `SEQUENCE_VALUE_ITEM` (
  `SEQ_NAME` varchar(60) NOT NULL COMMENT '表名称',
  `SEQ_ID` decimal(20,0) NOT NULL COMMENT '序列值',
  `LAST_UPDATED_STAMP` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
  `CREATED_STAMP` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`SEQ_NAME`),
  KEY `SQNC_VL_ITM_UPDATED` (`LAST_UPDATED_STAMP`),
  KEY `SQNC_VL_ITM_CREATED` (`CREATED_STAMP`)
...
) ENGINE=InnoDB AUTO_INCREMENT=101 DEFAULT CHARSET=utf8 COMMENT='内容分类';
